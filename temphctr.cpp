            ...
            int tag_len)
{
    ...
    for (int i = 0; i < 16; i++)
        ctx->key2[i] = key[i + 16];
    AES_set_encrypt_key(key1, 0, (AES_KEY *)ctx->KEY);
                  ...
                  ae_ctx *ctx)
{
    ...
    __m128i tmp1, tmp2, tweakswap, ciphertext, Z1Z2;
    __m128i H = _mm_loadu_si128((__m128i *)ctx->key2);
    H = _mm_shuffle_epi8(H, BSWAP_MASK);
    H = multx(H);
    __m128i lenswap = _mm_insert_epi64(lenswap, pt_len * 8, 0);
    // Hash1
...
int ae_decrypt(ae_ctx *ctx,
               ...
               u8 *pt)
{
    HCTR_decrypt(ct, pt, nonce, ct_len, ctx);
    return ct_len;
}
void HCTR_decrypt(const unsigned char *ct,
                  unsigned char *pt,
                  ...
                  ae_ctx *ctx)
{
    ...
    H = _mm_shuffle_epi8(H, BSWAP_MASK);
    H = multx(H);
    __m128i lenswap = _mm_insert_epi64(lenswap, ct_len * 8, 0);
    // Hash1
